    ...
    def clean_and_interpolate(self, data: List[float], sampling_rate: int = 50) -> Tuple[np.ndarray, List[Dict], List[int]]:
        ...
        for region_start, region_end in error_regions:
            ...
            if valid_before >= 0 and valid_after < n:
                ...
                # WICHTIG: Athlet kann nur VORWÄRTS laufen!
                # Wenn end_value < start_value, suche den nächsten gültigen Punkt der GRÖSSER ist
                if end_value <= start_value:
                    # Suche weiter nach einem gültigen Punkt der größer ist
                    search_idx = valid_after + 1
                    ...
                    # Falls kein größerer Wert gefunden, schätze basierend auf Geschwindigkeit
                    if not found_valid_end:
                        # Erwartete Position: Start + (Anzahl Frames * erwartete Schrittgröße)
                        num_frames = region_end - valid_before
                        end_value = start_value + num_frames * expected_step
                        # valid_after bleibt wie es war, aber end_value ist korrigiert
                
                # Berechne wie viele Punkte zwischen den gültigen Werten liegen sollten
                gap_size_mm = end_value - start_value
                
                # Sicherheitscheck: gap_size muss positiv sein!
                if gap_size_mm <= 0:
                    gap_size_mm = (valid_after - valid_before) * expected_step
